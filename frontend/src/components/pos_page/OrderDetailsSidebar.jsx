...
const OrderDetailsSidebar = ({
  ...
  // ----- Edit-Form props (pass from parent) -----
  showEditForm,
  setShowEditForm,
  editingItem,
  editQuantity,
  ...
  return (
    <>
      ...
      <div
        ...
        `}
      >
        {/* Header + Tabs + Order Info (fixed) */}
        <div className="p-4 sm:p-5 md:p-6 space-y-4 flex-shrink-0">
          {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto px-4 sm:px-5 md:px-6 pb-4">
        ...
        {currentOrderItems.length > 0 && (
          <div className="sticky bottom-0 bg-white border-t border-gray-200 shadow-lg">
            <div className="px-3 sm:px-4 py-3 space-y-2 text-xs sm:text-sm bg-gray-50">
              <div className="flex justify-between">
            ...
            <div className="flex justify-between items-center px-3 sm:px-4 py-2 border-t border-gray-200 bg-white">
            ...
            <div className="flex gap-2 sm:gap-3 px-3 sm:px-4 pb-4 pt-2 bg-white">
              <button
                style={{ backgroundColor: PRIMARY_BLUE }}
      ...
      {/* ==================== NOTE MODAL (no overlay bg) ==================== */}
      {showNoteForm && (
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md p-4 sm:p-5">
            ...
            <div className="mb-3">
              <textarea
                value={itemNote}
                onChange={(e) => {
                  if (e.target.value.length <= NOTE_CHARACTER_LIMIT) setItemNote(e.target.value);
                }}
            ...
            <div className="flex gap-2 sm:gap-3">
              <button
                onClick={() => setShowNoteForm(false)}
                className="flex-1 py-2.5 rounded-full bg-gray-200 text-gray-700 font-medium hover:bg-gray-300 transition text-sm"
              >
                Cancel
              </button>
              <button
                onClick={saveNote}
                style={{ backgroundColor: PRIMARY_BLUE }}
                className="flex-1 py-2.5 rounded-full text-white font-medium hover:opacity-90 transition text-sm"
              >
                Save Note
              </button>
            </div>
      ...
      {/* ==================== EDIT ITEM MODAL (exact screenshot style) ==================== */}
        ...
        <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-sm sm:max-w-md p-4 sm:p-5">
            ...
            {availableExtras.length > 0 && (
              <div className="mb-5">
                <p className="font-semibold text-gray-800 mb-2">Extras</p>
                <div className="space-y-2">
                  {availableExtras.map(extra => (
                    <label key={extra.id} className="flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={editExtras.includes(extra.id)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setEditExtras([...editExtras, extra.id]);
                          } else {
                            setEditExtras(editExtras.filter(id => id !== extra.id));
                          }
                        }}
            ...
            {availableRemovals.length > 0 && (
              <div className="mb-5">
                <p className="font-semibold text-gray-800 mb-2">Remove</p>
                <div className="space-y-2">
                  {availableRemovals.map(removal => (
                    <label key={removal.id} className="flex items-center gap-2 text-sm">
                      <input
                        type="checkbox"
                        checked={editRemovals.includes(removal.id)}
                        onChange={(e) => {
                          if (e.target.checked) {
                            setEditRemovals([...editRemovals, removal.id]);
                          } else {
                            setEditRemovals(editRemovals.filter(id => id !== removal.id));
                          }
                        }}
                        className="w-4 h-4 rounded border-gray-300"
                      />
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={saveEdit}
                className="flex-1 py-2.5 rounded-full bg-blue-600 text-white font-medium hover:bg-blue-700 transition text-sm"
              >
                Save
              </button>
            </div>
